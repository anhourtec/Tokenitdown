...
const colorBannerMap: Record<FormatFileProps, string> = {
  ...
  mp3: "bg-indigo-500 text-white",
  epub: "bg-violet-500 text-white",
};
...
export const FileCard = ({ formatFile, className }: FileCardProps) => {
  ...
  if (formatFile === "video" || formatFile === "mp3") {
    filePlaceholder = (
      <div className="bg-foreground/5 mb-1.5 space-y-1 rounded border p-1">
        <div className="flex justify-center gap-1">
  ...
  return (
    <div aria-hidden className={cn("relative size-fit", className)}>
      ...
      <div className="dark:bg-secondary ring-border relative z-1 h-18 w-14 space-y-3 rounded-md bg-white p-2 ring-1">
        {filePlaceholder}
      </div>
    </div>
  );
