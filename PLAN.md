...
- **Bundled MCP server** — Claude consumes MCP natively; ChatGPT supports MCP via connectors/Apps. One MCP server satisfies both — no separate integrations.
  - **✅ Conversion tools shipped** (`server/app/mcp_server.py`, FastMCP): `convert_url_to_markdown` (both modes), `convert_file_to_markdown` (stdio/local only — reads the user's own files), `convert_document` (HTTP/base64 upload). stdio for editors; streamable-HTTP for the hosted endpoint. Conversion engine is shared with the REST service via `server/app/conversion.py`.
  - **✅ Per-user API keys + transparency shipped:** keys minted/revoked from the dashboard (hashed SHA-256, shown once); in HTTP mode the MCP server validates a key and **proxies conversions through the web pipeline**, so agent conversions are cleaned, token-counted, saved to the Library, and tagged with the key (`document.apiKeyId`). The **Connect editor** page shows, per key, call count, tokens saved, and exactly what the agent converted. Convert routes (`/api/convert*`) accept `Authorization: Bearer tid_…`.
  - **✅ Drop-in agent files shipped:** downloadable `AGENTS.md` / `CLAUDE.md` / `skills.md` (`lib/agent-files.ts`, `/api/agent-files/[name]`), generated per-instance, so any agent (Codex, Cursor, Gemini, Windsurf, …) — not just Claude — knows to use TokenItDown. Viewable inline (GitHub-style) on the Connect page.
  - **TODO — library tools:** list documents, fetch Markdown by id, search library, fetch context bundle (DB access from the MCP layer).
